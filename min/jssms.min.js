...
"undefined"!=typeof $&&($.fn.Cb=function(c){function g(g){this.o=g;var k=this;g=$("<div></div>");var q=$('<div class="controls"></div>'),m=Cc(["fullscreenEnabled","mozFullScreenEnabled","webkitCancelFullScreen"]),r=Cc(["requestAnimationFrame","msRequestAnimationFrame","mozRequestAnimationFrame","webkitRequestAnimationFrame","oRequestAnimationFrame"],window),z;if(r)this.requestAnimationFrame=window[r].bind(window);else{var S=0;this.requestAnimationFrame=function(a){var c=vc(),g=Math.max(0,16-(c-S));
...
this.D[z].appendTo(q);this.log=$('<div id="status"></div>');this.screen.appendTo(g);this.yb.appendTo(g);q.appendTo(g);this.log.appendTo(g);g.appendTo($(a));c!=aa&&this.Gb(c);$(document).bind("keydown",function(a){k.o.U.keydown(a)}).bind("keyup",function(a){k.o.U.keyup(a)})}else $(a).html('<div class="alert alert-error"><strong>Oh no!</strong> Your browser doesn\'t support writing pixels directly to the <code>&lt;canvas&gt;</code> tag. Try the latest versions of Firefox, Google Chrome, Opera or Safari!</div>')}
