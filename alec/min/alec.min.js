...
!1;void 0==b&&(b=document);a.some(function(a){return a in b?(c=a,!0):!1});return c},isIE:function(){return/msie/i.test(navigator.userAgent)&&!/opera/i.test(navigator.userAgent)}};function BinaryRequest(a,b,c,e,d){a=a||"GET";var h=[];if(c){var k=[],q;for(q in c){var f=escape(q);void 0!=c[q]&&(f+="="+escape(c[q]));k.push(f)}b+="?"+k.join("&")}var g=new XMLHttpRequest;g.open(a,b,!0);try{g.responseType="arraybuffer"}catch(m){console.error("responseType arrayBuffer not supported!")}g.onreadystatechange=function(){if(4==g.readyState){var a;for(a=0;a<h.length;++a)h[a]()}};g.onload=function(){this.response instanceof ArrayBuffer?d(this.response):console.error("Bad response type: "+
typeof this.response+" for "+JSON.stringify(this.response))};g.send(e);this.always=function(a){4==g.readyState&&a();h.push(a);return this}}function SyncWriter(){this.kBufferLength=524288;this.syncBuffer=new Uint16Array(this.kBufferLength);this.fileOffset=this.syncBufferIdx=0;this.curRequest=null;this.buffers=[]}
...
incR:function(){this.r=this.r&128|this.r+1&127},generateFlagTables:function(){var a,b,c,e,d,h,k;for(a=0;256>a;a++)b=0!=(a&128)?F_SIGN:0,c=0==a?F_ZERO:0,e=a&32,d=a&8,h=this.getParity(a)?F_PARITY:0,this.SZ_TABLE[a]=b|c|e|d,this.SZP_TABLE[a]=b|c|e|d|h,this.SZHV_INC_TABLE[a]=b|c|e|d,this.SZHV_INC_TABLE[a]|=128==a?F_OVERFLOW:0,this.SZHV_INC_TABLE[a]|=0==(a&15)?F_HALFCARRY:0,this.SZHV_DEC_TABLE[a]=b|c|e|d|F_NEGATIVE,this.SZHV_DEC_TABLE[a]|=127==a?F_OVERFLOW:0,this.SZHV_DEC_TABLE[a]|=15==(a&15)?F_HALFCARRY:
0,this.SZ_BIT_TABLE[a]=0!=a?a&128:F_ZERO|F_PARITY,this.SZ_BIT_TABLE[a]|=e|d|F_HALFCARRY;a=0;b=65536;c=0;e=65536;for(h=0;256>h;h++)for(k=0;256>k;k++)d=k-h,this.SZHVC_ADD_TABLE[a]=0!=k?0!=(k&128)?F_SIGN:0:F_ZERO,this.SZHVC_ADD_TABLE[a]|=k&(F_BIT5|F_BIT3),(k&15)<(h&15)&&(this.SZHVC_ADD_TABLE[a]|=F_HALFCARRY),k<h&&(this.SZHVC_ADD_TABLE[a]|=F_CARRY),0!=((d^h^128)&(d^k)&128)&&(this.SZHVC_ADD_TABLE[a]|=F_OVERFLOW),a++,d=k-h-1,this.SZHVC_ADD_TABLE[b]=0!=k?0!=(k&128)?F_SIGN:0:F_ZERO,this.SZHVC_ADD_TABLE[b]|=
k&(F_BIT5|F_BIT3),(k&15)<=(h&15)&&(this.SZHVC_ADD_TABLE[b]|=F_HALFCARRY),k<=h&&(this.SZHVC_ADD_TABLE[b]|=F_CARRY),0!=((d^h^128)&(d^k)&128)&&(this.SZHVC_ADD_TABLE[b]|=F_OVERFLOW),b++,d=h-k,this.SZHVC_SUB_TABLE[c]=0!=k?0!=(k&128)?F_NEGATIVE|F_SIGN:F_NEGATIVE:F_NEGATIVE|F_ZERO,this.SZHVC_SUB_TABLE[c]|=k&(F_BIT5|F_BIT3),(k&15)>(h&15)&&(this.SZHVC_SUB_TABLE[c]|=F_HALFCARRY),k>h&&(this.SZHVC_SUB_TABLE[c]|=F_CARRY),0!=((d^h)&(h^k)&128)&&(this.SZHVC_SUB_TABLE[c]|=F_OVERFLOW),c++,d=h-k-1,this.SZHVC_SUB_TABLE[e]=
0!=k?0!=(k&128)?F_NEGATIVE|F_SIGN:F_NEGATIVE:F_NEGATIVE|F_ZERO,this.SZHVC_SUB_TABLE[e]|=k&(F_BIT5|F_BIT3),(k&15)>=(h&15)&&(this.SZHVC_SUB_TABLE[e]|=F_HALFCARRY),k>=h&&(this.SZHVC_SUB_TABLE[e]|=F_CARRY),0!=((d^h)&(h^k)&128)&&(this.SZHVC_SUB_TABLE[e]|=F_OVERFLOW),e++},getParity:function(a){var b=!0,c;for(c=0;8>c;c++)0!=(a&1<<c)&&(b=!b);return b},generateMemory:function(){if(SUPPORT_DATAVIEW)for(var a=0;8192>a;a++)this.memWriteMap.setUint8(a,0);else for(a=0;8192>a;a++)this.memWriteMap[a]=0;if(SUPPORT_DATAVIEW)for(a=
...
this.outputChannel[1]+this.outputChannel[2]+this.outputChannel[3];d>HI_BOUNDARY?d=HI_BOUNDARY:d<LO_BOUNDARY&&(d=LO_BOUNDARY);c[a+e]=d;this.clockFrac+=this.clock;var h=this.clockFrac>>SCALE,k=h<<SCALE;this.clockFrac-=k;this.freqCounter[0]-=h;this.freqCounter[1]-=h;this.freqCounter[2]-=h;this.freqCounter[3]=128==this.noiseFreq?this.freqCounter[2]:this.freqCounter[3]-h;for(d=0;3>d;d++){var q=this.freqCounter[d];if(0>=q){var f=this.reg[d<<1];6<f?(this.freqPos[d]=(k-this.clockFrac+(2<<SCALE)*q<<SCALE)*
this.freqPolarity[d]/(k+this.clockFrac),this.freqPolarity[d]=-this.freqPolarity[d]):(this.freqPolarity[d]=1,this.freqPos[d]=NO_ANTIALIAS);this.freqCounter[d]+=f*(h/f+1)}else this.freqPos[d]=NO_ANTIALIAS}0>=this.freqCounter[3]&&(this.freqPolarity[3]=-this.freqPolarity[3],128!=this.noiseFreq&&(this.freqCounter[3]+=this.noiseFreq*(h/this.noiseFreq+1)),1==this.freqPolarity[3]&&(d=0,d=0!=(this.reg[6]&4)?0!=(this.noiseShiftReg&FEEDBACK_PATTERN)&&0!=(this.noiseShiftReg&FEEDBACK_PATTERN^FEEDBACK_PATTERN)?
...
b+1]=this.CRAM[e+1],this.display[c+b+2]=this.CRAM[e+2]}else this.drawBGColour(a)}},drawBg:function(a){var b=0,c=0,e=0,d=0,h=this.vdpreg[8],k=ACCURATE?this.vScrollLatch:this.vdpreg[9];16>a&&0!=(this.vdpreg[0]&64)&&(h=0);var q=this.vdpreg[0]&128,f=32-(h>>3)+this.h_start,g=a+k>>3;27<g&&(g-=28);for(var k=(a+(k&7)&7)<<3,m=a<<8,l=this.h_start;l<this.h_end;l++){var b=this.bgt+((f&31)<<1)+(g<<6),p=this.VRAM[b+1],s=(p&8)<<1,r=(l<<3)+(h&7),t=0==(p&4)?k:56-k,u=this.tiles[(this.VRAM[b]&255)+((p&1)<<8)];if(0==
(p&2))for(b=0;8>b&&r<SMS_WIDTH;b++,r++)c=u[b+t],e=4*(r+m),d=3*(c+s),this.bgPriority[r]=0!=(p&16)&&0!=c,this.display[e]=this.CRAM[d],this.display[e+1]=this.CRAM[d+1],this.display[e+2]=this.CRAM[d+2];else for(b=7;0<=b&&r<SMS_WIDTH;b--,r++)c=u[b+t],e=4*(r+m),d=3*(c+s),this.bgPriority[r]=0!=(p&16)&&0!=c,this.display[e]=this.CRAM[d],this.display[e+1]=this.CRAM[d+1],this.display[e+2]=this.CRAM[d+2];f++;0!=q&&23==l&&(g=a>>3,k=(a&7)<<3)}},drawSprite:function(a){for(var b=0,c=0,e=0,d=0,h=this.lineSprites[a],
k=Math.min(SPRITES_PER_LINE,h[SPRITE_COUNT]),q=this.vdpreg[1]&1,f=a<<8,g=3*k;d<k;d++){var m=h[g--]|(this.vdpreg[6]&4)<<6,l=h[g--],p=h[g--]-(this.vdpreg[0]&8),b=a-l>>q;0!=(this.vdpreg[1]&2)&&(m&=-2);m=this.tiles[m+((b&8)>>3)];l=0;0>p&&(l=-p,p=0);var s=l+((b&7)<<3);if(0==q)for(;8>l&&p<SMS_WIDTH;l++,p++)b=m[s++],0==b||this.bgPriority[p]||(c=4*(p+f),e=3*(b+16),this.display[c]=this.CRAM[e],this.display[c+1]=this.CRAM[e+1],this.display[c+2]=this.CRAM[e+2],VDP_SPRITE_COLLISIONS&&(this.spriteCol[p]?this.status|=
STATUS_COLLISION:this.spriteCol[p]=!0));else for(;8>l&&p<SMS_WIDTH;l++,p+=2)b=m[s++],0==b||this.bgPriority[p]||(c=4*(p+f),e=3*(b+16),this.display[c]=this.CRAM[e],this.display[c+1]=this.CRAM[e+1],this.display[c+2]=this.CRAM[e+2],VDP_SPRITE_COLLISIONS&&(this.spriteCol[p]?this.status|=STATUS_COLLISION:this.spriteCol[p]=!0)),0==b||this.bgPriority[p+1]||(c=4*(p+f+1),e=3*(b+16),this.display[c]=this.CRAM[e],this.display[c+1]=this.CRAM[e+1],this.display[c+2]=this.CRAM[e+2],VDP_SPRITE_COLLISIONS&&(this.spriteCol[p+
1]?this.status|=STATUS_COLLISION:this.spriteCol[p+1]=!0))}h[SPRITE_COUNT]>=SPRITES_PER_LINE&&(this.status|=STATUS_OVERFLOW)},drawBGColour:function(a){a<<=8;var b=4*(a+4*SMS_WIDTH),c=3*((this.vdpreg[7]&15)+16);for(a*=4;a<b;a+=4)this.display[a]=this.CRAM[c],this.display[a+1]=this.CRAM[c+1],this.display[a+2]=this.CRAM[c+2]},decodeTiles:function(){for(var a=this.minDirty;a<=this.maxDirty;a++)if(this.isTileDirty[a]){this.isTileDirty[a]=0;for(var b=this.tiles[a],c=0,e=a<<5,d=0;d<TILE_SIZE;d++)for(var h=
this.VRAM[e++],k=this.VRAM[e++],q=this.VRAM[e++],f=this.VRAM[e++],g=128;0!=g;g>>=1){var m=0;0!=(h&g)&&(m|=1);0!=(k&g)&&(m|=2);0!=(q&g)&&(m|=4);0!=(f&g)&&(m|=8);b[c++]=m}}this.minDirty=TOTAL_TILES;this.maxDirty=-1},decodeSat:function(){this.isSatDirty=!1;for(var a=0;a<this.lineSprites.length;a++)this.lineSprites[a][SPRITE_COUNT]=0;a=0==(this.vdpreg[1]&2)?8:16;1==(this.vdpreg[1]&1)&&(a<<=1);for(var b=0;64>b;b++){var c=this.VRAM[this.sat+b]&255;if(208==c)break;c++;240<c&&(c-=256);for(var e=c;e<SMS_HEIGHT;e++)if(e-
c<a){var d=this.lineSprites[e];if(!d||d[SPRITE_COUNT]>=SPRITES_PER_LINE)break;var h=3*d[SPRITE_COUNT]+SPRITE_X,k=this.sat+(b<<1)+128;d[h++]=this.VRAM[k++]&255;d[h++]=c;d[h++]=this.VRAM[k]&255;d[SPRITE_COUNT]++}}},createCachedImages:function(){for(var a=0;a<TOTAL_TILES;a++)this.tiles[a]=new Uint8Array(TILE_SIZE*TILE_SIZE)},generateConvertedPals:function(){var a,b,c,e;for(a=0;64>a;a++)b=a&3,c=a>>2&3,e=a>>4&3,this.main_JAVA_R[a]=85*b&255,this.main_JAVA_G[a]=85*c&255,this.main_JAVA_B[a]=85*e&255;for(a=
...
window.$&&($.fn.JSSMSUI=function(a){var b=this,c=function(c){this.main=c;var d=this;c=$("<div></div>");var h=$('<div id="screen"></div>'),k=$('<div class="gamepad"><div class="direction"><div class="up"></div><div class="right"></div><div class="left"></div><div class="down"></div></div><div class="buttons"><div class="start"></div><div class="fire1"></div><div class="fire2"></div></div></div>'),q=$('<div id="controls"></div>');JSSMS.Utils.getPrefix(["fullscreenEnabled","mozFullScreenEnabled"]);var f=
JSSMS.Utils.getPrefix(["requestAnimationFrame","mozRequestAnimationFrame"],window),g;this.requestAnimationFrame=window[f].bind(window);this.screen=$("<canvas width="+SMS_WIDTH+" height="+SMS_HEIGHT+" moz-opaque></canvas>");this.canvasContext=this.screen[0].getContext("2d");this.canvasContext.mozImageSmoothingEnabled=!1;this.canvasContext.imageSmoothingEnabled=!1;this.canvasImageData=this.canvasContext.getImageData(0,0,SMS_WIDTH,SMS_HEIGHT);this.gamepad={u:{e:$(".up",k),k:KEY_UP},r:{e:$(".right",k),
k:KEY_RIGHT},d:{e:$(".down",k),k:KEY_DOWN},l:{e:$(".left",k),k:KEY_LEFT},1:{e:$(".fire1",k),k:KEY_FIRE1},2:{e:$(".fire2",k),k:KEY_FIRE2}};f=$(".start",k);$("#play").click(function(a){if(!d.main.romFileName)return a.preventDefault(),!1;d.main.isRunning?(d.main.stop(),d.updateStatus("Paused"),$("#play span").removeClass("icon-pause").addClass("icon-play")):(d.main.start(),$("#play span").removeClass("icon-play").addClass("icon-pause"))});var m="",l=new FileReader;l.onload=function(a){d.loadROMFile(a.target.result,
m)};$("#romUpload").change(function(){if(0!=document.getElementById("romUpload").files.length){var a=document.getElementById("romUpload").files[0];m=a.name;l.readAsBinaryString(a)}});this.romSelect=$("#romSelector").change(function(){d.loadROM()});$("#fullscreen").click(function(){var a=h[0];a.requestFullscreen?a.requestFullscreen():a.mozRequestFullScreen()});this.screen.appendTo(h);k.appendTo(h);h.appendTo(c);q.appendTo(c);c.appendTo($(b));void 0!=a&&this.setRoms(a);$(document).bind("keydown",function(a){d.main.keyboard.keydown(a)}).bind("keyup",
function(a){d.main.keyboard.keyup(a)});for(g in this.gamepad)this.gamepad[g].e.on("mousedown touchstart",function(a){return function(b){d.main.keyboard.controller1&=~a;b.preventDefault()}}(this.gamepad[g].k)).on("mouseup touchend",function(a){return function(b){d.main.keyboard.controller1|=a;b.preventDefault()}}(this.gamepad[g].k));f.on("mousedown touchstart",function(a){d.main.is_sms?d.main.pause_button=!0:d.main.keyboard.ggstart&=-129;a.preventDefault()}).on("mouseup touchend",function(a){d.main.is_sms||
(d.main.keyboard.ggstart|=128);a.preventDefault()})};c.prototype={reset:function(){this.screen[0].width=SMS_WIDTH;this.screen[0].height=SMS_HEIGHT},setRoms:function(a){var b,c,k,q,f=0;this.romSelect.children().remove();$("<option></option>").appendTo(this.romSelect);for(b in a){if(a.hasOwnProperty(b)){c=$("<optgroup></optgroup>").attr("label",b);k=a[b].length;for(q=0;q<k;q++)$("<option>"+a[b][q][0]+"</option>").attr("value",a[b][q][1]).appendTo(c);c.appendTo(this.romSelect)}f++}},loadROM:function(){var a=
...
$("#play").removeClass("disabled");$("#fullscreen").removeClass("disabled")},enable:function(){},updateStatus:function(){},writeAudio:function(a){},writeFrame:function(){var a=JSSMS.Utils.getPrefix(["hidden","mozHidden"]);return a?function(){document[a]||this.canvasContext.putImageData(this.canvasImageData,0,0)}:function(){this.canvasContext.putImageData(this.canvasImageData,0,0)}}(),updateDisassembly:function(a){for(var b=this.main.cpu.instructions,c=b.length,k="",q=8>a?0:a-8,f=0;16>f&&q<=c;q++)b[q]&&
(k+="<div"+(b[q].address==a?' class="current"':"")+">"+b[q].hexAddress+(b[q].isJumpTarget?":":" ")+"<code>"+b[q].inst+"</code></div>",f++);this.dissambler.html(k)}};return c});var IO_TR_DIRECTION=0,IO_TH_DIRECTION=1,IO_TR_OUTPUT=2,IO_TH_OUTPUT=3,IO_TH_INPUT=4,PORT_A=0,PORT_B=5;JSSMS.Ports=function(a){this.main=a;this.vdp=a.vdp;this.psg=a.psg;this.keyboard=a.keyboard;this.europe=64;this.hCounter=0;this.ioPorts=[]};
...
c.getUint16();break;case 160:break;case 161:break;case 162:break;case 163:break;case 168:break;case 169:break;case 170:break;case 171:break;case 176:g=c.position-2;l=!0;break;case 177:g=c.position-2;l=!0;break;case 178:g=c.position-2;l=!0;break;case 179:g=c.position-2;l=!0;break;case 184:break;case 185:break;case 186:g=c.position-2;l=!0;break;case 187:g=c.position-2;l=!0;break;default:JSSMS.Utils.console.error("Unexpected opcode","0xED "+e(d))}a.nextAddress=c.position;a.operand=f;a.target=g;a.isFunctionEnder=
...
a.left.name)&&(a.left=e[a.left.name]));return a});return a})}};return a}();var Generator=function(){function a(a){return JSSMS.Utils.traverse(a,function(a){"Register"==a.type&&(a.type="Identifier");return a})}var b=JSSMS.Utils.toHex,c="page temp location val value JSSMS.Utils.rndInt".split(" "),e=function(){this.ast=[]};e.prototype={generate:function(d){JSSMS.Utils.console.time("Generating");for(var e=0;e<d.length;e++)d[e]=d[e].map(function(d){var e=[],f=d[0].address,g=0;d=d.map(function(a){null==a.ast&&(a.ast=[]);if(ENABLE_SERVER_LOGGER)var c={type:"ExpressionStatement",
expression:{type:"CallExpression",callee:n.Identifier("sync"),arguments:[]}};var d=g,e=a.opcode,f=0;switch(e[0]){case 203:f=OP_CB_STATES[e[1]];break;case 221:case 253:f=2==e.length?OP_DD_STATES[e[1]]:OP_INDEX_CB_STATES[e[2]];break;case 237:f=OP_ED_STATES[e[1]];break;default:f=OP_STATES[e[0]]}g=d+f;d=[{type:"ExpressionStatement",expression:{type:"AssignmentExpression",operator:"-=",left:{type:"Identifier",name:"tstates"},right:{type:"Literal",value:g}}}];g=0;DEBUG&&(d[0].expression.right.raw=b(d[0].expression.right.value));
